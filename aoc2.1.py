...
def main():
    score = 0
    with open("aoc2_input.txt") as strategy:
        for match in strategy:
            match = match.strip()
            elf, player = match.split()
            score += points_for_shape[player]
            if beats_elf[elf] == player:
                score += 6
            elif draws_elf[elf] == player:
                score += 3
            else:
                score += 0
    print(f"Following this strategy guide, my total score would be {score}.")
